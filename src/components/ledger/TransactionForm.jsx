...
export default function TransactionForm(props) {
  ...
  const [activeDatePreset, setActiveDatePreset] = useState("NOW");
  const [showCustomDate, setShowCustomDate] = useState(false);

  useEffect(() => {
    if (showForm && !editingId && !txnDate) {
      applyDatePreset("NOW");
  ...
  const applyDatePreset = (preset) => {
    ...
    const now = new Date();
    if (preset === "NOW") {
      setTxnDate(getLocalDateTimeString(now));
      setShowCustomDate(false);
    } else if (preset === "YESTERDAY") {
      const yesterday = new Date(now);
      yesterday.setDate(now.getDate() - 1);
      setTxnDate(getLocalDateTimeString(yesterday));
      setShowCustomDate(false);
    } else if (preset === "CUSTOM") {
      if (!txnDate) setTxnDate(getLocalDateTimeString(now));
      setShowCustomDate(true);
    }
  };
    ...
    <>
      ...
      {!showForm && !editingId && canAdd && (
        <div className="fab-container">
          <button className="w-14 h-14 rounded-full bg-brandBlue text-white border-none shadow-md cursor-pointer flex justify-center items-center text-3xl transition-transform duration-200 hover:scale-105 hover:bg-blue-800" onClick={() => setShowForm(true)} aria-label="Add Transaction">
            +
          </button>
          
          <div className="flex gap-2 mb-2 opacity-100 translate-y-0 pointer-events-auto transition-all duration-200">
            <button className="px-4 py-2.5 rounded-3xl font-semibold text-sm border-none shadow-md cursor-pointer flex items-center gap-1.5 bg-cashOut text-white" onClick={() => handleQuickAdd("EXPENSE")}>
              - OUT
            </button>
            <button className="px-4 py-2.5 rounded-3xl font-semibold text-sm border-none shadow-md cursor-pointer flex items-center gap-1.5 bg-cashIn text-white" onClick={() => handleQuickAdd("INCOME")}>
              + IN
            </button>
          </div>
        </div>
        ...
        <form onSubmit={(e) => {
          ...
          <div className="grid gap-4">
            ...
            <div className="flex bg-black/20 rounded-lg p-1 gap-1">
              <button 
                type="button" 
                className={`flex-1 p-2.5 border-none rounded-md font-semibold text-sm cursor-pointer transition-all ${type === "INCOME" ? "bg-cashIn text-white shadow-sm" : "bg-transparent text-textMuted hover:text-white"}`}
              ...
              <button 
                type="button" 
                className={`flex-1 p-2.5 border-none rounded-md font-semibold text-sm cursor-pointer transition-all ${type === "EXPENSE" ? "bg-cashOut text-white shadow-sm" : "bg-transparent text-textMuted hover:text-white"}`}
            ...
            <div className="flex flex-col gap-1.5 overflow-hidden">
              <label className="text-[13px] font-semibold text-textMuted uppercase tracking-[0.5px]">Date & Time</label>
              ...
              <div className="flex items-center gap-1.5 bg-white/5 p-1 rounded-lg border border-borderLight overflow-x-auto max-w-full [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
                <button 
                  type="button"
                  onClick={() => applyDatePreset("NOW")} 
                  className={`px-3 py-1.5 rounded-md text-sm font-medium transition-all whitespace-nowrap ${activeDatePreset === "NOW" ? "bg-brandBlue text-white shadow-md" : "text-textMuted hover:text-white hover:bg-white/5"}`}
                >
                  Now
                </button>
                <button 
            ...
            <div className="flex flex-col gap-1.5 mt-2">
              <label className="text-[13px] font-semibold text-textMuted uppercase tracking-[0.5px]">Receipt (Optional)</label>
            ...
            <button 
              type="submit" 
              className={`btn-primary mt-4 text-lg p-4 transition-opacity ${type === "INCOME" ? "!bg-cashIn" : "!bg-cashOut"} ${saving ? "opacity-70" : "opacity-100"}`}
              disabled={saving} 
            >
              {saving ? "Saving..." : (editingId ? "Update Entry" : `Save ${type === "INCOME" ? "Cash IN" : "Cash OUT"}`)}
            </button>
